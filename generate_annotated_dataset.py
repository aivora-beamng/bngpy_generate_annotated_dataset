    ...
    for curr_map in MAPS_TO_GENERATE:
        ...
        with tqdm(total=IMGS_PER_MAP) as pbar:
            ...
            while i < IMGS_PER_MAP:
                ...
                data['colour'].convert('RGB').save(
                    Path(cam_dir, f'{curr_map}_{i:06}.png'))
                data['annotation'].save(
                    Path(ann_dir, f'{curr_map}_{i:06}_annotation.png'))
                ...
                if dist < MINIMAL_MOVE_DISTANCE:
                    ...
                    print(
                        f'Vehicle did not move (dist={dist:.2f}). {steps_without_moving_left} steps until reset...')
                else:
                    jammed = False
                    steps_without_moving_left = MAX_STEPS_WITHOUT_MOVING
                    i += 1
                ...
                if steps_without_moving_left == 0:
                    print(
                        f'Traffic jam detected at iteration {i:06}, resetting traffic vehicles...')
                    steps_without_moving_left = MAX_STEPS_WITHOUT_MOVING + 1
                    unjam_traffic(beamng, ego, curr_map, jammed)
                    jammed = True
                last_pos = pos
